...
const CheckOutSucess = () => {
  ...
  useEffect(() => {
    axios.delete(`http://localhost:3500/CartReset/${email}`)
    ...
    const fetchOrderData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:3500/SuccessfulPayment/api/stripe/order?session_id=${sessionId}`); // Replace '/api/stripe/order' with your backend endpoint for fetching order data
        setOrderData(response.data);
        setIsLoading(false);
        const emlrs = await axios.get(`http://localhost:3500/email/send/${response.data.id}`);
        console.log(emlrs);
      } catch (error) {
